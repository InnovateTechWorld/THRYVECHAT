...
const Billing = () => {
  ...
  try {
    ...
    if (status === 'successful' && (transaction_id || tx_ref)) {
      ...
      // ✅ USE YOUR EXISTING verifyPayment FUNCTION
      ...
      <div className="flex-1 p-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{billingData?.billing?.plan_name || 'Free Plan'}</div>
                      <p className="text-xs text-muted-foreground">
                        {Number(billingData?.billing?.plan_price || 0) > 0 ? (
                          <>
                            ${formatCurrency(billingData?.billing?.plan_price)}/month
                            {billingData?.billing?.current_period_end && (
                              <> • Renews {formatDate(billingData.billing.current_period_end)}</>
                            )}
                          </>
                        ) : (
                          'Free tier'
                      ...
                      <div className="mt-2">
                        <Badge 
                          variant={billingData?.billing?.subscription_status === 'active' ? 'default' : 'secondary'}
                        >
                          {billingData?.billing?.subscription_status || 'inactive'}
                        </Badge>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-green-600">
                        ${formatCurrency(billingData?.billing?.available_credits)}
                      </div>
                      <p className="text-xs text-muted-foreground">
                        ${formatCurrency(billingData?.billing?.total_purchased)} purchased • ${formatCurrency(billingData?.billing?.total_used)} used
                      </p>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">
                        ${formatCurrency(billingData?.billing?.total_spent)}
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {billingData?.billing?.total_transactions || 0} transactions
                      </p>
                    </CardContent>
                  </Card>
                </div>
                ...
                {/* Recent Transactions */}
                {(billingData?.recentTransactions?.length || 0) > 0 && (
                  <Card>
                    <CardHeader>
                ...
                {billingData?.billing?.subscription_status === 'active' && Number(billingData?.billing?.plan_price || 0) > 0 && (
          ...
          <TabsContent value="subscription">
            <SubscriptionPlans 
              currentPlan={billingData?.billing.plan_name}
              onPlanSelect={() => loadBillingData()}
            />
          </TabsContent>

          {/* Credits Tab */}
          <TabsContent value="credits">
            <CreditTopup onTopupComplete={() => loadBillingData()} />
          </TabsContent>
